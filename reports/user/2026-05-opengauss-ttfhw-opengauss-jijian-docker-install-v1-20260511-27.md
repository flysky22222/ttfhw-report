# TTHFW 用户场景验证报告

## 一、概述

- 目标：`openGauss 7.0.0-RC3 极简版 Docker 镜像`
- 时间：2026-05-11T09:00:00 ~ 2026-05-11T09:55:00（3300 秒）
- 镜像：`` —— 历史 md 报告未结构化记录镜像

## 二、环境

- 容器 OS：｜架构：｜Python：

## 三、文档阅读 / 抽取

- **test_goal**：作为 openGauss 用户，按 openGauss 官网最新版本（7.0.0-RC3）的安装文档（架构 x86，操作系统 openEuler 24.03 LTS）安装网页上的极简版 Docker 镜像，完成下载、安装、使用，发现安装文档缺陷。　（来源：测试目标）
- **doc_completeness**：仅靠官方文档无法独立完成，需修正镜像名后才能运行容器　（来源：1.2 测试总结）

## 四、四阶段 / 结果

| 阶段 | 状态 | 耗时(s) | 断点 |
|------|------|--------|------|
| 了解 | 成功 | 900 | 0 |
| 安装 | 成功 | 1500 | 1 |
| 使用 | 成功 | 600 | 0 |
| 贡献 | 失败 | 300 | 1 |

## 五、文档缺陷清单

- Docker 镜像名不一致（严重）
- 下载 URL 基础路径不一致（中等）
- 版本号示例过期（低）
- Docker 版本操作系统不一致（低）
- 镜像名不一致、下载 URL 路径不一致、版本示例过期

## 七、结论

安装验证成功，但文档存在镜像名不一致缺陷，用户按文档原命令执行会失败

> 本 md 由 `tools/ttfhw/cli/render_report.py` 从标准 JSON 报告渲染，JSON 为唯一真值源。